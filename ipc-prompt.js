...
You reply to your operator the normal way — your ordinary response text reaches them as it always does. Inside clodex you additionally can message the other agents and manage your own session. Both work through the intent lines below: include the matching line in your response to trigger it. To reach another agent, write the intent line rather than a plain sentence (a normal "ask bob to …" just goes to your operator; the intent line is what hands it to bob). Write it yourself — no echo/printf or shell wrapper needed.
  ...
  [agent:notify-user] message      Raise a note into the operator's persistent INBOX to get their attention when you're BLOCKED on a decision only they can make. Fires an OS notification and shows in the inbox with your name until they read it — the inbox lives behind the Inbox button in the sidebar footer (also File ▸ Inbox); tell your operator to look there if they ask where the note went. Use SPARINGLY — it interrupts them; it's for a genuine decision-block, not status updates or things a teammate agent can answer. Free-text, may span lines. Silent on success; an empty or oversized (>16KB) note bounces.
...
const GRAMMAR_LINES = [
  ...
  { type: 'notify-user', text: `  [agent:notify-user] message      Raise a note into the operator's persistent INBOX to get their attention when you're BLOCKED on a decision only they can make. Fires an OS notification and shows in the inbox with your name until they read it — the inbox lives behind the Inbox button in the sidebar footer (also File ▸ Inbox); tell your operator to look there if they ask where the note went. Use SPARINGLY — it interrupts them; it's for a genuine decision-block, not status updates or things a teammate agent can answer. Free-text, may span lines. Silent on success; an empty or oversized (>16KB) note bounces.` },
