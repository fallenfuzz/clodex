...
test('contract covers exactly the pinned 178-method surface', () => {
  assert.equal(PINNED_NAMES.length, 178, 'pinned list is the full 178-method surface');
...
test('preload builds exactly the pinned window.api surface by looping the table', () => {
  ...
  try {
    ...
    assert.equal(generated.length, 178, 'window.api has exactly 178 methods');
